...
public class RESTServerCommunicator implements ServerCommunicator {
    ...
    /* TODO: create a clien once. use credentials sent with the request */
    ...
    public String registerHost() throws NotRegisteredException {
        ...
        client.addFilter(new HTTPBasicAuthFilter("tomek", "tomek"));
    ...
    public void registerVm(VirtualMachine vm) throws NotRegisteredException {
        ...
        ClientResponse response = webRes.path("hosts").path(
                Properties.getHostName()).path("vms").type(
                MediaType.APPLICATION_XML).entity(vm)
    ...
    public void registerService(String vmid, Service service)
        ...
        WebResource webRes = client.resource(baseURI);
        ClientResponse response = webRes.path("hosts").path(
                Properties.getHostName()).path("vms").path(vmid).path(
                "services").type(MediaType.APPLICATION_XML).entity(service)
                .post(ClientResponse.class);
        if (response.getStatus() >= 300) {
